...
- `prototype-layout.js` 负责后台原型的顶部导航和左侧菜单注入。
- `prototype-switcher.js` 负责后台原型的状态切换浮窗能力。
- `README.md` 维护后台公共组件清单、接入契约和禁止项。

## 归位规则
- 后台页面级 HTML 原型统一引用当前目录下的公共资源。
- 后台新增可复用的原型组件能力，优先沉淀到当前目录维护，而不是再单独扩散出一套并行的“通用组件”目录。
- Excel 模板、示例文件等原型配套附件不放在当前目录，统一归位到 [../模板文件/](../模板文件/)。
- App 端组件仍然独立维护在 [../../移动端/通用组件/](../../移动端/通用组件/)。

## 维护约定
- 修改后台原型公共能力时，同步更新本 README 和 [../后台管理-组件预览.html](../后台管理-组件预览.html)。
- 页面私有样式只允许用于明显业务特化内容。
- 后台原型路径统一使用 `04-原型效果图/后台管理/...`。
- 业务规则主定义仍以 [../../../02-PRD文档/后台管理/](../../../02-PRD文档/后台管理/) 为准。
- 已经犯过的原型误错，不只当场纠正；应同步登记到 [../../../03-工作台/错误治理清单.md](../../../03-工作台/错误治理清单.md) 并决定是否升级为模板或脚本检查。
...
2. 读取 `DESIGN-admin.md` 设计规范，确认色板、字体、间距、阴影、圆角策略等视觉基线。
3. 再查看本文件中的组件清单与最小结构。
4. 再查看 `后台管理-组件预览.html` 中是否已有完整组合示例。
5. 先列“页面元素 → 组件名”映射，再开始修改页面。
6. 仅当现有组件无法覆盖时，才允许新增页面私有实现，并注明原因。
7. 新页面优先从 [templates/](templates/) 下的起手模板开始，而不是从空白 HTML 手写。

组件样式的色值、字重、阴影层级、圆角口径必须遵循 `DESIGN-admin.md` 中的 Color Palette、Typography、Depth & Elevation 定义，不允许偏离。
...
## Golden Path 起手模板
- 列表页：优先从 [templates/列表页起手模板.html](templates/列表页起手模板.html) 起手。
- 详情/编辑页：优先从 [templates/详情页起手模板.html](templates/详情页起手模板.html) 起手。
- 弹窗表单：优先复用 [templates/弹窗表单起手模板.html](templates/弹窗表单起手模板.html) 片段。
- 模板的目标是让新页面先站到共享组件骨架上，再补业务特化内容。

## 筛选区宽度规则
- 后台列表页筛选区默认优先使用 `proto-filter-bar` 的自然排布，不先写自定义拉伸网格。
- 若确实需要单页自定义筛选网格，2-4 个字段时优先使用固定宽度带：
  - 搜索输入：`200px ~ 240px`
  - 普通下拉：`160px ~ 220px`
  - 级联选择：`200px ~ 240px`
  - 查询 / 重置按钮：`auto`
- 禁止用 `1fr / 1.2fr / 1.25fr` 这类比例列把少量筛选字段横向拉满；这种写法会让字段看起来过宽、头部松散，且不同页面宽度感难统一。
- 只有当筛选项明显超过一行、或存在宽文本字段必须拉伸时，才允许引入弹性列，并应在页面备注中说明原因。

## 高频误错 -> 制度化动作
| 高频误错 | 默认制度化动作 |
|---|---|
| 已有列表骨架仍手写页面结构 | 先补“页面元素 -> 组件名”映射，再从列表页模板起手；必要时运行 `check_prototypes.py` |
| 操作列重新手写多个文字链接 | 优先改为 `proto-row-actions + proto-action`，并由原型检查脚本兜底 |
| 少量筛选字段被 `fr` 比例列拉得过宽 | 优先改回固定宽度带；必要时由原型检查脚本给出轻量告警 |
| 页面底部按钮自行写 sticky/fixed 样式 | 改为 `page-actions + has-page-actions`，禁止页面私有定位补丁 |
| 页面有状态切换浮窗但未接入共享脚本 | 改为 `prototype-state-bar + prototype-switcher.js` 组合 |
| 页面有 frame/topbar/sidebar 占位但未接入共享布局脚本 | 改为 `prototype-layout.js` 注入导航和侧栏 |
